...
function marketDetailMarkup(market) {
  ...
  const nextMove = roundToHalf(nextLine - market.currentLine);
  const comparisonWidth = comparisonPercent(market.currentLine, market.seasonAverage);
  const nextMoveClass = nextMove > 0 ? "move-up" : nextMove < 0 ? "move-down" : "move-flat";
  const nextMoveArrow = nextMove > 0 ? "\u2191" : nextMove < 0 ? "\u2193" : "\u2192";
  ...
  const nextPair = linePairForMidpoint(nextLine);
  return `<div class="hero-head"><div><div class="hero-player">${market.playerName}</div><div class="hero-meta">${market.team} | ${market.position}</div></div><span class="status-chip ${status.className}">${status.label}</span></div><div class="hero-price"><div class="hero-price-value">${market.currentLine.toFixed(1)}</div><div class="hero-delta ${movement.className}">${movement.arrow} ${movement.label}</div></div><div class="hero-comparison"><div class="hero-comparison-copy"><span>U ${currentPair.underLine.toFixed(1)} | O ${currentPair.overLine.toFixed(1)}</span><strong class="${nextMoveClass}">${nextMoveArrow} Next midpoint ${nextLine.toFixed(1)}</strong></div><div class="hero-comparison-bar"><span class="hero-comparison-fill" style="width:${comparisonWidth}%"></span></div></div><form id="trade-form" class="trade-form-grid ${isPending ? "is-pending" : ""}"><div class="trade-actions"><button type="button" class="trade-button trade-over ${uiState.selectedSide === "OVER" ? "active" : ""}" data-side="OVER" ${isPending ? "disabled" : ""}><span class="trade-button-price">OVER ${currentPair.overLine.toFixed(1)}</span><span class="trade-button-hint">Matches opposite user liquidity</span></button><button type="button" class="trade-button trade-under ${uiState.selectedSide === "UNDER" ? "active" : ""}" data-side="UNDER" ${isPending ? "disabled" : ""}><span class="trade-button-price">UNDER ${currentPair.underLine.toFixed(1)}</span><span class="trade-button-hint">Matches opposite user liquidity</span></button></div><div class="quick-stakes"><button type="button" class="quick-stake-button" data-stake-add="5" ${isPending ? "disabled" : ""}>+5</button><button type="button" class="quick-stake-button" data-stake-add="10" ${isPending ? "disabled" : ""}>+10</button><button type="button" class="quick-stake-button" data-stake-add="25" ${isPending ? "disabled" : ""}>+25</button><button type="button" class="quick-stake-button" data-stake-max="true" ${isPending ? "disabled" : ""}>MAX</button></div><div class="stake-row"><label>Stake<input id="stake-input" name="stake" type="number" min="1" step="1" value="${stakeDraft}" required ${isPending ? "disabled" : ""}></label><div class="stake-preview"><span>Return</span><strong id="stake-return">${formatStake(stakeDraft * 2)}</strong></div></div><p class="trade-hint">Current spread: Under ${currentPair.underLine.toFixed(1)} / Over ${currentPair.overLine.toFixed(1)}. If unmatched, your order stays pending until another user takes the other side.</p><button class="primary-button ${isPending ? "is-loading" : ""}" type="submit" ${isPending ? "disabled" : ""}>${isPending ? "Placing..." : "Confirm position"}</button><p id="trade-feedback" class="feedback" aria-live="polite">${isPending ? "Submitting trade..." : ""}</p></form>`;
}
