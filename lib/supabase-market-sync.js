...
async function persistSupabaseMarketState(localMarket,targetState){
  ...
  await upsertRows("matched_pairs",(localMarket.matchedPairs||[]).map((pair)=>buildMatchedPairRow(pair,marketContext,context,usersByName)).filter(Boolean),"id");
  await syncMarketHoldings(localMarket,marketContext,context,usersByName);
  await syncWalletLedger(targetState,context.round.id,usersByName,tradeIdsByLocalId);
  await upsertRows("wallet_snapshots",userNames.map((userName)=>{
...
async function syncWalletLedger(targetState,roundId,usersByName,tradeIdsByLocalId){
  ...
  await upsertRows("wallet_ledger",ledgerRows,"external_ledger_key");
}
...
async function syncMarketHoldings(localMarket,marketContext,context,usersByName){
  ...
  if(desiredHoldings.length){
    await upsertRows("holdings",desiredHoldings,"user_id,market_id,side");
  }
  if(staleIds.length){
    await supabaseRequest("holdings",{
      method:"DELETE",
