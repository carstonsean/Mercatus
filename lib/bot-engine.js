...
function createRandomBot(existingBots = []) {
  ...
  return {
    ...
    userName: `Quick Take Bot ${customCount}`,
    archetype: "custom",
    source: "custom",
    ...
    config: {
      ...
      label: "Custom",
      seasonWeight: 1,
      lastGameWeight: randomBetween(0, 1),
      last3Weight: randomBetween(0, 1),
      venueWeight: randomBetween(0, 1),
      opponentWeight: randomBetween(0, 1),
      matchupWeight: randomBetween(0, 1),
      noiseLevel: 0,
      edgeThreshold: randomBetween(1, 2.4),
      activityRate: randomBetween(0.55, 0.95),
      cooldownTicks: Math.round(randomBetween(2, 5)),
      oversBias: 0,
      starBias: 0,
      crowdBias: 0
    },
...
function buildReasonSummary({ market, bot, projection, side }) {
  ...
  return `${botLabel} bot projected ${projection.value.toFixed(1)} vs market ${market.currentLine.toFixed(1)} driven mainly by ${drivers}. LG ${projection.factors.effectiveLastGameFormMultiplier.toFixed(2)}x, L3 ${projection.factors.effectiveLast3FormMultiplier.toFixed(2)}x, HA ${projection.factors.effectiveHomeAwayMultiplier.toFixed(2)}x, Opp ${projection.factors.effectiveOpponentDifficultyMultiplier.toFixed(2)}x, Pos ${projection.factors.effectivePositionalImpactMultiplier.toFixed(2)}x. Took $1 ${side.toLowerCase()}.`;
}
